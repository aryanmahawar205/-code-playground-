...
// A simple class with no dynamically allocated memory
class BasicNumber
{
public:
  ...
  // n by *doubling* (in this case) the other object's n member variable.  If we do not create 
...
// The Number class has a member variable that points to dynamically
